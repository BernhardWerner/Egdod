...
(function(){
	...
	drawOutline(image, size, color) := (
		...
		colorplot(
			vis =       sum(apply(1..a, imagergba(image, [x,y] + size *         [sin(2 * pi * # / a), cos(2 * pi * # / a)])_4));
			vis = vis + sum(apply(1..b, imagergba(image, [x,y] + size * 2 / 3 * [sin(2 * pi * # / b + 2), cos(2 * pi * # / b + 2)])_4));
			vis = vis + sum(apply(1..c, imagergba(image, [x,y] + size     / 3 * [sin(2 * pi * # / c + 4), cos(2 * pi * # / c + 4)])_4));

			(color.x, color.y, color.z, 1) * vis;
		);
	);
	...
	rotate(point, alpha) := rotate(point, alpha, [0,0]);
		...
		************************************************************************************************* */
		...
		catchSliderRaw(slider) := (
			...
			if(lineSegmentSDF(endPoints, mouse().xy) <= slider.bulbSize + 0.02 * slider.size,
		...
		*************************************************************************************************/
		...
		catchSelectorRaw(selector) := (
			...
			if(lineSegmentSDF(endPoints, mouse().xy) <= selector.bulbSize + 0.02 * selector.size,
		...
		************************************************************************************************* */
		...
		drawSpectrumSlider(slider) := (
			...
			colorplot(
				...
				col = wavelength2rgb(lerp(380, 780, lambda));
			...
			fillpoly(handleRect, color -> wavelength2rgb(lerp(380, 780, slider.value))_[1,2,3]);
		...
		catchSpectrumSliderRaw(slider) := (
			...
			if(lineSegmentSDF(endPoints, mouse().xy) <= slider.handleWidth,
			  slider.value = if(slider.vertical,
				clamp(inverseLerp((endPoints_1).y, (endPoints_2).y, mouse().y), 0, 1);
			  , // else //
				clamp(inverseLerp((endPoints_1).x, (endPoints_2).x, mouse().x), 0, 1);
		...
		wavelength2rgb(lambda) := (
			// Taken from https://codepen.io/pen/?editors=0110
			// Based on http://www.midnightkite.com/color.html

			regional(factor, red, green, blue);
			...
			,if((lambda >= 420) & (lambda < 701),
				factor = 1.0;
			,if((lambda >= 701) & (lambda < 781),

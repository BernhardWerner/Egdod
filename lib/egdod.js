	...
	rotate(point, alpha) := rotate(point, alpha, [0,0]);
		...
		************************************************************************************************* */
		...
		drawSpectrumSlider(slider) := (
			regional(endPoints, sliderRect, handleRect, vert, lambda);
			...
			colorplot(
				...
				wavelength2rgb(lerp(380, 780, lambda));
			);
			greset();

			drawpoly(sliderRect, color -> slider.color, size -> 3);
			fillpoly(handleRect, color -> wavelength2rgb(lerp(380, 780, slider.value)));
		...
		wavelength2rgb(lambda) := (
			inverseLerp(380, 780, lambda) * (1,1,1);
		);

...
(function(){
	...
	evalLUTbackwards(lut, y) := evalLUTforwards(apply(lut, [#.y, #.x]), y);
		...
		drawCone3D(base, radius, tip, color) := drawCone3D(base, radius, tip, color, 64);
	...
	drawAxes3D(start, end) := drawAxes3D(start, end, 0.5, 0.4, 0.15);
...
*/
		...
		wavelength2rgba(lambda) := (
			...
			distSquared(u, v) := sum(apply(1..length(u), (u_# - v_#)^2));
